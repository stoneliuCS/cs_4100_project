...
def parse_block_address(block_address: str) -> dict:
    ...
    if "&" in block_address:
        ...
        if len(parts) >= 2:
            suffix = (
                parts[-1]
                if parts[-1] in ["ST", "AVE", "RD", "BLVD", "DR", "LN", "WAY", "PL"]
                else ""
            )
    ...
    street_name = (
        " ".join(parts[street_start_idx:street_end_idx])
        if street_end_idx > street_start_idx
        else ""
    )
...
def generate_block_samples(
    ...
    block_num = block_num if block_num is not None else 0
...
def batch_geocode_addresses(addresses_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(0, num_rows, BATCH_SIZE):
        ...
        temp_csv = (
            Path(__file__).parent / f"temp_geocoding_batch_{i // BATCH_SIZE + 1}.csv"
        )
...
def geocode_block_samples(
    ...
    formatted_addresses = [
        format_address_for_geocoding(addr, city, state, zip_code)
    ...
    geocoding_df = pd.DataFrame(
        {
            ...
            "zip": [zip_code] * len(sample_addresses)
            if zip_code
            else [""] * len(sample_addresses),
        }
    )
        ...
        results.append(
            {
                ...
                "sample_address": sample_addresses[idx]
                if idx < len(sample_addresses)
                else "",
                "formatted_address": formatted_addresses[idx]
                if idx < len(formatted_addresses)
                else "",
                "lat": lat,
                "lon": lon,
...
def process_all_block_addresses(
    ...
    result_df = result_df[
        result_df["geocode_status"].isin(["Match", "Tie"])
        & result_df["lat"].notna()
        & result_df["lon"].notna()
    ]
